...
app.post('/puzzles', async (c) => {
  ...
  try {
    const { meta: { last_row_id } } = await c.env.DB.prepare('INSERT INTO puzzles (puzzle_name, puzzle_size, created_at, updated_at) VALUES (?, ?, datetime("now"), datetime("now"))').bind(name, size).run();
